/*
 * TestCasesPage4 — Evals 2: Test Cases
 *
...
function PassFailBadge({ result }: { result: 'pass' | 'fail' | 'running' | 'unknown' }) {
  const c = {
    pass: { icon: <CheckCircle2 size={12} />, label: 'Pass', cls: 'text-green-600 dark:text-green-400 bg-green-500/10 border-green-500/20' },
    fail: { icon: <XCircle size={12} />, label: 'Fail', cls: 'text-red-600 dark:text-red-400 bg-red-500/10 border-red-500/20' },
    running: { icon: <Loader2 size={12} className="animate-spin" />, label: 'Running', cls: 'text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
  ...
  const handleImportFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.created > 0) {
        ...
        navigate(`/evals2/benchmarks/${bm.id}/runs`);
      }
  ...
  const renderTcRow = (tc: TestCase, indent: boolean = false) => {
    ...
    return (
      <tr key={tc.id} className="border-b hover:bg-muted/50 cursor-pointer transition-colors"
        onClick={() => navigate(`/evals2/test-cases/${tc.id}`)}>
            ...
            <div className="absolute top-8 left-3 z-20 bg-popover border border-border rounded-md shadow-lg p-2 min-w-[160px]">
              {bmList.map(bm => (
                <div key={bm.id} className="text-xs px-2 py-1.5 rounded hover:bg-muted cursor-pointer flex items-center gap-1.5"
                  onClick={e => { e.stopPropagation(); navigate(`/evals2/benchmarks/${bm.id}/runs`); }}>
                  <Badge className="text-[8px] px-1 py-0 bg-purple-500/15 text-purple-400 border-purple-500/30 shrink-0">BM</Badge>{bm.name}
                </div>
              ))}
            </div>
          )}
                    ...
                    groupedData.map(group => {
                      ...
                      return (
                        <React.Fragment key={group.id}>
                          {/* Group header */}
                          <tr className={`border-b cursor-pointer hover:bg-muted/30 transition-colors ${isUnassigned ? 'bg-muted/10' : 'bg-purple-500/5'}`}
                            ...
                            <td colSpan={5} className="px-3 py-2 align-middle">
                              <div className="flex items-center gap-2">
                                ...
                                {!isUnassigned && <Badge className="text-[9px] px-1 py-0 bg-purple-500/15 text-purple-400 border-purple-500/30 font-semibold uppercase tracking-wider">Benchmark</Badge>}
                    ...
                    {filteredRuns.length === 0 ? (
                      ...
                      return (
                        <tr key={run.id} className="border-b hover:bg-muted/50 cursor-pointer transition-colors"
                          onClick={() => tcId && navigate(`/evals2/test-cases/${tcId}`)}>
